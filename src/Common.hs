...
buildFilename identifiers extension = fromText (T.intercalate "-" (map lineToText identifiers)) <.> extension
...
aggregateFilePath p = do
  (parent . parent) p </> aggregateFileName p

toIncludeFiles :: Map.Map FilePath [FilePath] -> Map.Map FilePath Text
toIncludeFiles fileMap = Map.map generatedIncludeText fileMap

generatedIncludeText :: [FilePath] -> Text
generatedIncludeText files = do
  let lines = map (format ("!include "%fp)) files
  T.intercalate "\n" $ includePreamble:(lines ++ [""])

includePreamble = "### Generated by hledger-makeitso - DO NOT EDIT ###\n"

writeIncludeFiles :: [FilePath] -> Shell FilePath
writeIncludeFiles = writeFiles . toIncludeFiles . groupValuesBy aggregateFilePath
...
writeFiles' fileMap = Map.foldlWithKey (\a k v -> writeTextFile k v) (return ()) fileMap
