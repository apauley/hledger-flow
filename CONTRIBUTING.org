...
- [[https://github.com/apauley/hledger-flow/issues?q=author%3Adesaiuditd][Udit Desai]] (Github Action for build, test and release)
...
6. [[#releases][Understand the Release Process]]
...
The current automation is tag-driven:

- =bin/tag-release= creates the signed git tag from the version in =package.yaml=
- when a version tag is pushed, the workflow builds release artifacts and creates a /draft/ GitHub release containing the packaged binaries

Release metadata currently comes from =package.yaml= and =ChangeLog.md=.
Before pushing a release tag, make sure those files agree on the version number.

** For Contributors
   :PROPERTIES:
   :CUSTOM_ID: for-outside-contributors
   :END:

Contributors are not expected to publish releases, push release tags, or upload to Hackage.

What is helpful in contributor pull requests:

- keep changes releasable: tests passing, docs updated, and user-visible changes described in =ChangeLog.md=
- see if it is possible to update the stack dependencies: update =resolver= in =stack.yaml= with a newer version from [[https://www.stackage.org/][Stackage]]
- if you are preparing a release-related pull request, update =package.yaml= and =ChangeLog.md= together
- if you want to help validate Hackage packaging, also verify that a source tarball can be produced locally:
...
This project keeps =package.yaml= in git and ignores the generated =hledger-flow.cabal= file.
If =cabal check= or =cabal sdist= complains that the cabal file is missing or stale, regenerate it first by running =stack test= (or another =stack= command that refreshes the cabal file from =package.yaml=).

** For Maintainers
   :PROPERTIES:
   :CUSTOM_ID: for-maintainers
   :END:

The release procedure for this project is:
...
#+END_SRC
   ...
   Update the cabal bounds as described below if =cabal check= reports any warnings.
...
** Updating Cabal Bounds For Release
   :PROPERTIES:
   :CUSTOM_ID: updating-cabal-bounds-for-release
   :END:

This project develops against the Stackage snapshot in =stack.yaml=,
but the Hackage package metadata comes from the generated cabal file.
Before a Hackage release, update the dependency bounds in =package.yaml= so that they reflect the
versions currently selected by Stack.

Use this process:

1. Confirm the resolver in =stack.yaml=.
2. Ask Stack for the direct dependency versions in the current snapshot:

#+BEGIN_SRC sh
    stack ls dependencies --depth 1
#+END_SRC

3. For each direct dependency in =package.yaml=, set a lower bound at the version reported by Stack and set an upper bound at the next PVP-compatible series.
...
4. Regenerate the cabal file and verify the metadata:
...
Keep the bounds broad enough to avoid unnecessary churn, but do not leave them unbounded when publishing to Hackage.
The goal is to describe the dependency range that is known to work for the released source package, based on the Stack snapshot used for that release.
